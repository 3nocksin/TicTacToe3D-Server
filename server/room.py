...
class GameRoom:
    ...
    def add_player(
        self, websocket: WebSocket, name: str, preferred_symbol: str | None
    ) -> PlayerConnection:
        if self.is_full:
            raise ValueError("La sala ya está llena.")

        requested = (preferred_symbol or "").strip().upper()
        if requested and requested not in {"X", "O"}:
            raise ValueError("Debes elegir X u O.")
        if requested and requested in self.players:
            raise ValueError(f"El símbolo {requested} ya fue elegido en esta sala.")

        symbol = requested or ("X" if "X" not in self.players else "O")
        player_number = 1 if symbol == "X" else 2
        normalized_name = name.strip()[:20] or f"Jugador {player_number}"
        ...
        self.players[symbol] = connection
        return connection

    def player_name(self, symbol: str | None) -> str:
        if symbol in self.players:
            return self.players[symbol].name
        return f"Jugador {1 if symbol == 'X' else 2}"

    def player_label(self, symbol: str) -> str:
        return f"{self.player_name(symbol)} ({symbol})"
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            ...
            "players": {
                symbol: {"name": player.name, "player_number": player.player_number}
                for symbol, player in self.players.items()
            },
...
class RoomManager:
    ...
    async def create_room(
        ...
        async with self.lock:
            ...
            player = room.add_player(websocket, name, preferred_symbol)
            self.rooms[code] = room
            self.connection_rooms[self._connection_key(websocket)] = code
            return room, player

    async def join_room(
        self, websocket: WebSocket, room_code: str, name: str, preferred_symbol: str | None
    ) -> tuple[GameRoom, PlayerConnection]:
