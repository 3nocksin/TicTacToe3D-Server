...
app = FastAPI(title="Tic Tac 3D", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def player_preferences(message: dict[str, Any]) -> tuple[str, str | None]:
    name = str(message.get("player_name", ""))
    preferred_symbol = str(message.get("preferred_symbol", "")).strip().upper() or None
    return name, preferred_symbol


async def handle_create_room(websocket: WebSocket, message: dict[str, Any]) -> None:
    if manager.room_for(websocket) is not None:
        await send_error(websocket, "Ya estás dentro de una sala.")
        return

    name, preferred_symbol = player_preferences(message)
    try:
        room, player = await manager.create_room(websocket, name, preferred_symbol)
    except ValueError as exc:
        await send_error(websocket, str(exc), "create_error")
        return
    await websocket.send_json(
        {
            "type": "room_created",
            "message": f"Sala creada. {player.name} jugará con {player.symbol}.",
            "room_code": room.code,
...
async def handle_join_room(websocket: WebSocket, message: dict[str, Any]) -> None:
    ...
    name, preferred_symbol = player_preferences(message)
    try:
        room, player = await manager.join_room(websocket, room_code, name, preferred_symbol)
    except ValueError as exc:
    ...
    await room.broadcast(
        {
            "type": "game_started",
            "message": (
                f"{room.player_label('X')} contra {room.player_label('O')}. "
                f"Comienza {room.player_label('X')}."
            ),
            "state": room.snapshot(),
            "rematch": False,
        }
...
async def handle_move(websocket: WebSocket, message: dict[str, Any]) -> None:
    ...
    async with room.lock:
        ...
        await room.broadcast(
            {
                "type": "state_update",
                "message": f"{room.player_label(symbol)} colocó en ({x}, {y}, {z}).",
        ...
        if result.winner:
            await room.broadcast(
                {
                    "type": "game_over",
                    "message": f"Ganó {room.player_label(result.winner)}.",
        ...
        await room.broadcast(
            {
                "type": "rematch_status",
                "message": (
                    f"{room.player_label(symbol)} solicitó revancha. "
                    f"{len(room.rematch_votes)} de 2 jugadores aceptaron la revancha."
                ),
                "accepted": len(room.rematch_votes),
                "required": 2,
                "requested_by": {
                    "symbol": symbol,
                    "player_number": 1 if symbol == "X" else 2,
                    "name": room.player_name(symbol),
                },
                "state": room.snapshot(),
            }
        )
